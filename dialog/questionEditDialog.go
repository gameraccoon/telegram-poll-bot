	...
	return Dialog {
		...
		variants : map[string]Variant {
			...
			"co" : Variant {
				...
				isActive : func(data *processing.ProcessData) {
...
func setTextCommand(data *processing.ProcessData) {
	if data.Static.Db.IsUserEditingQuestion(data.UserId) {
		...
		processing.SendMessage(data.Static.Bot, data.ChatId, data.Static.Trans("ask_question_text"))
	} else {
		processing.SendMessage(data.Static.Bot, data.ChatId, data.Static.Trans("warn_not_editing_question"))
	}
}
...
func setVariantsCommand(data *processing.ProcessData) {
	if data.Static.Db.IsUserEditingQuestion(data.UserId) {
		...
		processing.SendMessage(data.Static.Bot, data.ChatId, data.Static.Trans("ask_variants"))
	} else {
		processing.SendMessage(data.Static.Bot, data.ChatId, data.Static.Trans("warn_not_editing_question"))
	}
}
...
func setRulesCommand(data *processing.ProcessData) {
	if data.Static.Db.IsUserEditingQuestion(data.UserId) {
		...
		processing.SendMessage(data.Static.Bot, data.ChatId, data.Static.Trans("ask_rules"))
	} else {
		processing.SendMessage(data.Static.Bot, data.ChatId, data.Static.Trans("warn_not_editing_question"))
	}
}
...
func commitQuestionCommand(data *processing.ProcessData) {
	if data.Static.Db.IsUserBanned(data.UserId) {
		processing.SendMessage(data.Static.Bot, data.ChatId, data.Static.Trans("warn_youre_banned"))
		if data.Static.Db.IsUserEditingQuestion(data.UserId) {
			questionId := data.Static.Db.GetUserEditingQuestion(data.UserId)
	...
	if data.Static.Db.IsUserEditingQuestion(data.UserId) {
		...
		if data.Static.Db.IsQuestionReady(questionId) && data.Static.Db.GetQuestionVariantsCount(questionId) > 0 {
			...
			processing.SendMessage(data.Static.Bot, data.ChatId, data.Static.Trans("warn_question_not_ready"))
		}
	} else {
		processing.SendMessage(data.Static.Bot, data.ChatId, data.Static.Trans("warn_not_editing_question"))
	}
...
func discardQuestionCommand(data *processing.ProcessData) {
	if data.Static.Db.IsUserEditingQuestion(data.UserId) {
		questionId := data.Static.Db.GetUserEditingQuestion(data.UserId)
		data.Static.Db.DiscardQuestion(questionId)
		processing.SendMessage(data.Static.Bot, data.ChatId, data.Static.Trans("say_question_discarded"))
		processing.ProcessNextQuestion(data)
	} else {
		processing.SendMessage(data.Static.Bot, data.ChatId, data.Static.Trans("warn_not_editing_question"))
	}
...
func getEditingGuide(data *ProcessData) string {
	questionId := data.Static.Db.GetUserEditingQuestion(data.UserId)
